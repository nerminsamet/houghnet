...
def calculate_logmap(im_size, center, angle=90, R2_list = [4, 64, 256, 1024]):

    points = generate_grid(im_size[0], im_size[1]) #[x,y]
...
# vr = varying res
def calculate_logmap_vr(im_size, center, angles=[360, 30, 60, 90, 180], R2_list=[4, 64, 256, 1024, 100000000]):
   points = generate_grid(im_size[0], im_size[1]) #[x,y]
   # check inside which circle
   y_dif = points[:, 1].cuda() - center[0].float()
   x_dif = points[:, 0].cuda() - center[1].float()
   ...
   arc_angle = (torch.atan2(y_dif, x_dif) * 180 / PI).long()
   arc_angle[arc_angle < 0] += 360
   c_region = torch.ones(xdif_2.shape, dtype=torch.long).cuda() * (len(R2_list)-1)
   results = torch.zeros(xdif_2.shape, dtype=torch.long).cuda()
   total_angles = [360.0 / i for i in angles]
   total_regions = 0
   bottom = 0
   for ind, i in enumerate(R2_list):
       angle_id = (arc_angle / angles[ind]).long() + 1
       region = i
       mask = ((sum_of_squares) <= region) & ((sum_of_squares) > bottom)
       c_region[mask] = ind
       results[mask] = total_regions + angle_id[mask] + (c_region[mask] - 1) #* total_angles[ind]
       total_regions += total_angles[ind] -1
       bottom = i
   return results.view(im_size[0], im_size[1])
